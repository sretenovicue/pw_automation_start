...
test("Register user", async ({ page }) => {
  ...
  const automationExerciseHeaderText = page
    .getByRole("link")
  ...
  const newUserSignupTittle = page.getByRole("heading", {
  ...
  const accountInformationTittle = page.getByRole("heading", {
  ...
  const signUpForNewsletterCheckBox = page.getByLabel(
    "Sign up for our newsletter!"
  );

  await signUpForNewsletterCheckBox.click();

  const receiveSpecialOffersCheckBox = page.getByLabel(
